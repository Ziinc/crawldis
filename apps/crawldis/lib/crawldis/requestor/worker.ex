defmodule Crawldis.Requestor.Worker do
    ...
    case RequestQueue.pop_claimed_request() do
      {:ok, request} ->
        ...
        with {:ok, request_with_response} <- do_requesting(request),
             {:ok, %{items: items, requests: new_requests}} <-
               do_parsing(request_with_response) do
          ...
          {:error, _} = err ->
            Logger.error(
              "Unknown error occured while making request: #{inspect(err)}"
            )
  ...
  defp do_parsing(request_with_response) do
    case pipe(request_with_response.extractors, %Crawldis.Parsed{}, %{
           passthrough: false
         }) do
      {false, _} ->
